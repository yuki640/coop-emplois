...
class Smarty extends Smarty_Internal_TemplateBase
{
    ...
    */
    ...
    public function getPluginsDir()
    {
        ...
        if (!$this->_pluginsDirNormalized) {
            ...
            foreach ($this->plugins_dir as $k => $v) {
                $this->plugins_dir[ $k ] = $this->_realpath(phprtrim($v ?? '', '/\\') . DIRECTORY_SEPARATOR, true);
            }
    ...
    public function _realpath($path, $realpath = null)
    {
        ...
        $path = $parts[ 'path' ];
        if ($parts[ 'root' ] === '\\') {
            $parts[ 'root' ] = phpsubstr(getcwd(), 0, 2) . $parts['root'];
        } else {
            if ($realpath !== null && !$parts[ 'root' ]) {
                $path = phpgetcwd() . DIRECTORY_SEPARATOR . $path;
            }
        }
    ...
    {
        $this->{$dirName} = $this->_realpath(phprtrim($dir ?? '', "/\\") . DIRECTORY_SEPARATOR, true);
    }
    ...
    private function _normalizeTemplateConfig($isConfig)
    {
        ...
        foreach ($dir as $k => $v) {
            if (!isset($processed[ $k ])) {
                $dir[ $k ] = $v = $this->_realpath(phprtrim($v ?? '', "/\\") . DIRECTORY_SEPARATOR, true);
                $processed[ $k ] = true;
            }
        }
